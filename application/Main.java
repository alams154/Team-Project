...
public class Main extends Application {
	...
	/**
	 * This method takes in a path to a json file and parses it by saving its data
	 * to an internal hashTable structure
	 * 
	 * @param filepath
	 * @throws IOException
	...
	/*
	 * Method that creates and supports the first screen, which is in charge of
	 * choosing a file, exiting, or proceeding to the second screen
	 */
	...
	/*
	 * Method that creates and supports the second screen, which is in charge of
	 * adding, displaying, and writing data
	 */
	...
	/*
	 * Method that creates the third screen, which is the screen that displays if
	...
	/*
	 * Method that creates the fourth screen, which is the exit screen displayed
	...
	/*
	 * Method that creates the background for a specific screen
	 * (Used to setup background for all screens)
	 */
	...
	private void displayButtonAction(Stage primaryStage, BorderPane pane) {
		if (fileRead) {
			try {
				...
				TextField enterID = (TextField) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(0))
						.getChildren().get(1)).getChildren().get(0);
				CheckBox ra = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
						.get(0)).getChildren().get(0);
				CheckBox dec = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
						.get(0)).getChildren().get(1);
				CheckBox hflux = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
						.get(0)).getChildren().get(2);
				CheckBox sflux = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
						.get(1)).getChildren().get(0);
				CheckBox z = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
						.get(1)).getChildren().get(1);
				CheckBox rmag = (CheckBox) ((VBox) ((HBox) ((VBox) pane.getRight()).getChildren().get(1)).getChildren()
				...
				if (hflux.isSelected()) {
					...
					if (allData.get(searchID).getHflux() != -1.0) {
						horiz = new Text(params[3] + ": " + allData.get(searchID).getHflux() + " cm^(-2) sec^(-1)");
					} else {
				...
				if (sflux.isSelected()) {
					...
					if (allData.get(searchID).getSflux() != -1.0) {
						soft = new Text(params[4] + ": " + allData.get(searchID).getSflux() + " cm^(-2) sec^(-1)");
					} else {
	...
	/*
	 * Method that writes the sources that are stored in the second global hash
	 * table to a separate file that will be given a valid name as a parameter
	 */
	...
	/**
	 * The main method initializes the data structure back ends, and several misc.
	 * fields, and then runs the GUI
	 * 
	 * @param args
